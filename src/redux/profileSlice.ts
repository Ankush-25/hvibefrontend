...
const usrProfileSlice = createSlice({
    ...
    reducers: {
        ...
        deleteNestedFields: (state, action: PayloadAction<DeleteNestedFieldsPayload>) => {
            const { section, index } = action.payload;
            (state.profile as any)[section] = (state.profile as any)[section].filter((_: any, i: number) => (i !== index));
        },
        updateNestedFields: (state, action: PayloadAction<UpdateNestedFieldsPayload>) => {
