  ...
  return (
    <div className="bg-white dark:bg-gradient-to-br dark:from-secondary-900 dark:to-secondary-800 rounded-[20px] shadow-lg hover:shadow-xl dark:shadow-[0_10px_40px_rgba(0,0,0,0.3)] p-[25px] h-[300px] flex flex-col transition-all duration-400 ease-[cubic-bezier(0.4,0,0.2,1)] relative border border-primary-100 dark:border-white/5 overflow-hidden backdrop-blur-md hover:-translate-y-2 dark:hover:shadow-[0_15px_50px_rgba(0,0,0,0.5),0_0_25px_rgba(var(--color-secondary-500),0.25)] hover:border-secondary-500/50 group mx-2">
      ...
      <div className="flex justify-between items-start mb-5 relative z-[2] after:content-[''] after:absolute after:-bottom-3 after:left-0 after:w-[60px] after:h-[3px] after:bg-gradient-to-r after:from-secondary-500 after:to-primary-500 after:rounded-[3px] after:transition-[width] after:duration-300 group-hover:after:w-[80px]">
        <div className="text-[1rem] font-bold text-primary-800 dark:text-primary-50 leading-[1.4] max-w-[80%] w-48 truncate">
          {data.title}
        </div>
      </div>

      <div className="my-[5px] flex flex-col gap-[10px]">
        <div className="flex items-center gap-[10px]">
          <FontAwesomeIcon icon={faBuilding} className="text-secondary-500 text-[0.9rem] w-[18px] text-center" />
          <span className="text-[1rem] text-primary-600 dark:text-primary-200 truncate">{data.company}</span>
        </div>
        <div className="flex items-center gap-[10px]">
          <FontAwesomeIcon icon={faMapMarkerAlt} className="text-secondary-500 text-[0.9rem] w-[18px] text-center" />
          <span className="text-[1rem] text-primary-600 dark:text-primary-200 truncate">{data.location}</span>
        </div>
      </div>

      <div className="flex flex-wrap gap-[10px] my-5 relative">
        <span className="bg-primary-100 dark:bg-white/5 text-primary-700 dark:text-primary-200 px-[14px] py-[6px] rounded-[50px] text-[0.75rem] flex items-center gap-[6px] border border-primary-200 dark:border-white/5 transition-all duration-300 backdrop-blur-[5px] hover:bg-secondary-500/20 hover:border-secondary-500/30 hover:-translate-y-[2px] max-w-[140px]">
          <FontAwesomeIcon icon={faGraduationCap} className="flex-shrink-0" />
          <span className="truncate">{data.education || "Any Graduate"}</span>
        </span>
        <span className="bg-primary-100 dark:bg-white/5 text-primary-700 dark:text-primary-200 px-[14px] py-[6px] rounded-[50px] text-[0.75rem] flex items-center gap-[6px] border border-primary-200 dark:border-white/5 transition-all duration-300 backdrop-blur-[5px] hover:bg-secondary-500/20 hover:border-secondary-500/30 hover:-translate-y-[2px]">
      ...
      <div className="mt-auto flex justify-between items-center relative z-[2] pt-[15px] border-t border-primary-200 dark:border-white/5">
        <div className="font-bold text-primary-900 dark:text-primary-50 text-[1.1rem]">{data.salary || "₹2.5L - ₹5L/year"}</div>
        <button
          className="bg-gradient-to-br from-secondary-500 to-primary-500 text-primary-50 border-none px-[24px] py-[10px] rounded-[50px] font-semibold cursor-pointer flex items-center gap-[8px] transition-all duration-400 ease-[cubic-bezier(0.4,0,0.2,1)] relative overflow-hidden shadow-md before:content-[''] before:absolute before:inset-0 before:bg-gradient-to-br before:from-primary-500 before:to-secondary-500 before:opacity-0 before:transition-opacity before:duration-400 hover:before:opacity-100 hover:-translate-y-[2px] hover:shadow-lg group/btn"
          onClick={handleApplyClick}
        >
          <FontAwesomeIcon icon={faArrowRight} className="relative z-[1] transition-transform duration-300 group-hover/btn:translate-x-[3px]" />
...
export function FreshersJobsCarousel() {
  ...
  if (loading) {
    return (
      <div className="py-[80px] pb-[120px] bg-primary-50 dark:bg-gradient-to-br dark:from-[#1a1a1a] dark:to-[#242424] relative overflow-hidden before:content-[''] before:absolute before:inset-x-0 before:top-0 before:h-[2px] before:bg-gradient-to-r before:from-transparent before:via-secondary-500 before:to-transparent before:z-[1]">
        <h2 className="text-[2.5rem] font-extrabold text-center mb-[15px] text-primary-900 dark:text-primary-50 relative inline-block left-1/2 -translate-x-1/2 after:content-[''] after:absolute after:-bottom-2 after:left-0 after:w-full after:h-[3px] after:bg-gradient-to-r after:from-secondary-500 after:to-primary-500 after:rounded-[3px]">Freshers Jobs</h2>
        <div className="text-center p-[40px] text-[1.2rem] text-primary-500 dark:text-primary-400">Loading freshers jobs...</div>
      </div>
    );
  }
